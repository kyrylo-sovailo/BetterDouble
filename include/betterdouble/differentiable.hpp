...
namespace bd
{
    ...
    template<class T, unsigned int N> inline Differentiable<T, N> sqrt (const Differentiable<T, N> &x)                                          noexcept { Differentiable<T, N> v; v.value = std::sqrt(x.value);                   for (unsigned int i = 0; i < N; ++i) v.derivative[i] = x.derivative[i] / (2 * std::sqrt(v.value)); return v; };
    template<class T, unsigned int N> inline Differentiable<T, N> cbrt (const Differentiable<T, N> &x)                                          noexcept { Differentiable<T, N> v; v.value = std::cbrt(x.value);                   for (unsigned int i = 0; i < N; ++i) v.derivative[i] = x.derivative[i] / (3 * std::cbrt(v.value) * std::cbrt(v.value)); return v; };
